...
namespace LibApp.Application.Services
{
    public class BorrowService : IBorrowService
    {
        ...
        public async Task<ResultDto<BorrowDtoRes>> ReturnBook(Guid borrowId)
        {
            var result = new ResultDto<BorrowDtoRes>(false);
            var borrow = await _unitOfWork.Repository<Borrow>().GetByIdAsync(borrowId);
            if (borrow == null)
            {
                result.ReturnFail("Not Found");
                return result;
            ...
            borrow.Price = CalculatePrice(borrow);
            result.ReturnSuccess(borrow.Adapt<BorrowDtoRes>());
            return result;
        }
        ...
        {
            var totalDate = (DateTime.Now - borrow.Date).Days;
            return totalDate*PRICE_ONE_DAY;
        }
    }
}